...
duration-200"
            ...
            <ul>
                {livres.map((livre) => (
                    <li key={livre.id} className="py-2 flex justify-between w-full">
                        <span>
                            <strong>{livre.titre}</strong>
                        </span>
                        <span className="flex gap-2">
                            <Link className="text-blue-700 underline hover:no-underline"
                                href={`/${livre.id}/edit`}>Edit</Link>
                            <Link className="text-red-500 underline hover:no-underline"
                                href={`/${livre.id}/delete`}>Delete</Link>
                        </span>
                    </li>
                ))}
                {livres?.length < 1 && <div className="py-2">No data</div>}
            </ul>
        </div>
    )
}
